 ...
 */
class SrcJSON {
    /** @string srcPath 源码包 JSON 文件绝对路径。（默认为 src.json） */
    srcPath = SrcJSON.setPath('src.json');

    /**
     * 构造 JSON 文件操作实例。
     * @param {string} srcPath src JSON 文件路径。
     */
    constructor(srcPath) {
        if (srcPath) {
    ...
    add(importPath, srcPath, isBuild, cmd) {
        ...
        console.info('A package have been appended!');
    }
    ...
    remove(importPath) {
        ...
        console.info('A package have been removed!')
    }
}
